...
DATA_APOTEKER_CSV = "data_apoteker.csv"
DATA_OBAT_CSV = "data_obat.csv"
DATA_PASIEN_CSV = "data_pasien.csv"
DATA_STOCKOBAT_CSV = "data_stockobat.csv"

def baca_data_apoteker():
    if not os.path.exists(DATA_APOTEKER_CSV):
        return [] 

    try:
        df = pd.read_csv(DATA_APOTEKER_CSV)
        return df.to_dict(orient="records")
    except pd.errors.EmptyDataError:
        return []

def tulis_data_apoteker(data):
    df = pd.DataFrame(data)  
    df.to_csv(DATA_APOTEKER_CSV, index=False)

def baca_data_obat():
    if not os.path.exist(DATA_OBAT_CSV):
        return []
...
def tulis_data_pasien():
    if not os.path.exists(DATA_PASIEN_CSV):
        return []

def baca_data_pasien():
    if not os.path.exist(DATA_PASIEN_CSV):
        return []

    try:
        df = pd.read_csv(DATA_PASIEN_CSV)
...
def tulis_data_stockobat():
    df = pd.DataFrame(data)
    df.to_csv(DATA_STOCKOBAT_CSV, index=False)

def baca_data_stockobat():
    if not os.path.exist(DATA_STOCKOBAT_CSV):
        return []

    try:
        df = pd.read_csv(DATA_STOCKOBAT_CSV)
        return df.to_dict(orient="record")
    except pd.errors.EmptyDataError:
        return []
